...
def register():
    ...
    # Set default alert_message
    alert_message = {
        'status': session.get("alert_status"),
        'message': session.get("alert_message"),
    }

    # Clear session after using alert message
    session.pop("alert_status", None)
    session.pop("alert_message", None)

    return render_template('register.html', account_number=account_number, alert_message=alert_message)
...
def create():
    if request.method == "POST":
        ...
        sql_check = "SELECT * FROM atm_member WHERE username = %s"
        my_cursor.execute(sql_check, (username,))
        result = my_cursor.fetchone()
        
        if result:
            # Username already exists
            session['alert_status'] = "fail"
            session['alert_message'] = "Username already exists. Please choose a different one."
